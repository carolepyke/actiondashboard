...
const DARK   = "#1A1A2E";
const CREAM  = "#F9F3E3";
const MUTED  = "rgba(240,234,214,0.4)";
    ...
    return (
      <div style={{ display: "flex", gap: 10, padding: "8px 14px", alignItems: "center" }}>
        <input
          ...
          autoFocus
          style={{
            flex: 1, background: "rgba(255,255,255,0.06)", border: `1px solid ${accent}`,
  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{
        ...
        borderRadius: 6, background: hover ? "rgba(255,255,255,0.04)" : "transparent",
        transition: "background 0.15s", cursor: "default",
      }}>
      {/* Checkbox */}
      <div onClick={onToggle} style={{
        width: 20, height: 20, flexShrink: 0, borderRadius: 4, marginTop: 2, cursor: "pointer",
        border: `1.5px solid ${task.done ? accent : "rgba(255,255,255,0.18)"}`,
        ...
        {task.done && (
          <svg width="11" height="8" viewBox="0 0 11 8" fill="none">
            <path d="M1 3.5L4 6.5L10 1" stroke={DARK} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        )}
      </div>
      {/* Text */}
      <span onClick={onToggle} style={{
        flex: 1, fontSize: 14, fontFamily: "Arial, sans-serif", lineHeight: 1.55, cursor: "pointer",
...
function Btn({ children, onClick, accent = GOLD, ghost = false, small = false, danger = false }) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{
        background: bg, border: `1px solid ${danger ? "rgba(192,57,43,0.4)" : ghost ? "rgba(255,255,255,0.1)" : `${accent}44`}`,
  ...
  return (
    <button onClick={onClick} title={title}
      ...
      style={{
        ...
        cursor: "pointer", fontSize: 13, lineHeight: 1, transition: "background 0.15s",
      }}>
      {children}
    </button>
  );
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 18, lineHeight: 1 }}>{section.emoji || "📋"}</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: "bold", color: CREAM, fontFamily: "Arial" }}>{section.name}</div>
          <div style={{ fontSize: 10, color: MUTED, fontFamily: "Arial", marginTop: 2 }}>
        ...
        <div style={{ position: "relative", width: 36, height: 36 }}>
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 9, fontFamily: "Arial", fontWeight: "bold", color: accent }}>{p}%</div>
        </div>
        <div onClick={e => { e.stopPropagation(); onEditSection(); }} style={{ cursor: "pointer", fontSize: 14, opacity: 0.5, padding: "2px 4px" }} title="Edit section">✏️</div>
        <div onClick={e => { e.stopPropagation(); onDeleteSection(); }} style={{ cursor: "pointer", fontSize: 14, opacity: 0.5, padding: "2px 4px" }} title="Delete section">🗑️</div>
        <div style={{ fontSize: 12, color: MUTED, marginLeft: 2 }}>{collapsed ? "▼" : "▲"}</div>
      </div>
            ...
            {adding ? (
              <div style={{ display: "flex", gap: 8 }}>
                <input value={newTask} onChange={e => setNewTask(e.target.value)}
                  ...
                  style={{
                    flex: 1, background: "rgba(255,255,255,0.05)", border: `1px solid ${accent}44`,
              ...
              <button onClick={() => setAdding(true)} style={{
                ...
                fontSize: 12, cursor: "pointer", padding: "4px 6px", letterSpacing: 0.5,
              }}>+ Add task</button>
            )}
          </div>
        </div>
  ...
  return (
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: "#16182E", border: `1px solid ${BORDER}`,
        borderRadius: 12, padding: "32px 28px", width: "100%", maxWidth: 480,
        boxShadow: "0 24px 60px rgba(0,0,0,0.5)",
      }}>
        <div style={{ fontSize: 18, color: CREAM, marginBottom: 24, fontFamily: "Georgia, serif" }}>{title}</div>
        {children}
      </div>
    </div>
  );
...
function Field({ label, children }) {
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ fontSize: 10, letterSpacing: 3, color: MUTED, fontFamily: "Arial", marginBottom: 8 }}>{label}</div>
      {children}
    </div>
  );
}
...
export default function DashboardBuilder() {
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {EMOJIS.map(e => (
          <button key={e} onClick={() => onChange(e)} style={{
            width: 36, height: 36, borderRadius: 6, border: `1px solid ${e === value ? GOLD : "rgba(255,255,255,0.1)"}`,
            background: e === value ? "rgba(201,168,76,0.15)" : "rgba(255,255,255,0.03)",
  ...
  if (loaded && !userName) {
    return (
      <div style={{ minHeight: "100vh", background: DARK, fontFamily: "Georgia, serif", color: CREAM, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ position: "fixed", inset: 0, pointerEvents: "none", background: `radial-gradient(ellipse 55% 45% at 10% 5%, rgba(201,168,76,0.08) 0%, transparent 100%)` }}/>
        <div style={{ position: "relative", textAlign: "center", maxWidth: 400, padding: 32 }}>
          <div style={{ fontSize: 40, marginBottom: 20 }}>✨</div>
          <div style={{ fontSize: 9, letterSpacing: 4, color: GOLD, fontFamily: "Arial", opacity: 0.7, marginBottom: 10 }}>WORDS THAT DELIVER</div>
          <h1 style={{ fontSize: 26, fontWeight: "normal", margin: "0 0 8px", color: CREAM }}>The Personal Branding SPA</h1>
          <p style={{ fontSize: 14, color: GOLD, fontStyle: "italic", margin: "0 0 36px" }}>Dashboard Builder</p>
          <p style={{ fontSize: 14, color: MUTED, fontFamily: "Arial", lineHeight: 1.7, margin: "0 0 28px" }}>
          ...
          <input value={nameInput} onChange={e => setNameInput(e.target.value)}
            ...
            style={{
              ...
              color: CREAM, fontFamily: "Arial", fontSize: 15, outline: "none", marginBottom: 14,
              textAlign: "center",
            }}/>
  ...
  return (
    <div style={{ minHeight: "100vh", background: DARK, fontFamily: "Georgia, serif", color: CREAM }}>
      ...
      <div style={{
        ...
        background: `radial-gradient(ellipse 55% 45% at 10% 5%, rgba(201,168,76,0.07) 0%, transparent 100%),
      ...
      <div style={{ position: "relative", display: "flex", minHeight: "100vh" }}>
        ...
        {/* ── SIDEBAR ─────────────────────────────────────────────── */}
        <div style={{
          width: 240, flexShrink: 0, borderRight: `1px solid ${BORDER}`,
          padding: "36px 0 24px", display: "flex", flexDirection: "column",
          background: "rgba(0,0,0,0.15)",
        }}>
          <div style={{ padding: "0 20px 24px" }}>
            <div style={{ fontSize: 9, letterSpacing: 4, color: GOLD, fontFamily: "Arial", opacity: 0.7, marginBottom: 6 }}>
              WORDS THAT DELIVER
            </div>
            <div style={{ fontSize: 15, color: CREAM }}>Dashboard Builder</div>
          </div>

          <div style={{ flex: 1, overflowY: "auto", padding: "0 12px" }}>
            {dashboards.length === 0 && (
              <div style={{ padding: "16px 8px", fontSize: 12, color: MUTED, fontFamily: "Arial", lineHeight: 1.6 }}>
                ...
                <div key={d.id} onClick={() => setActiveDashId(d.id)} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, color: isActive ? GOLD : CREAM, fontFamily: "Arial", fontWeight: isActive ? "bold" : "normal",
          ...
          <div style={{ padding: "16px 12px 0" }}>
            <button onClick={() => { setForm({ name: "", emoji: "✨", color: GOLD, sub: "" }); setShowNewDash(true); }}
              style={{
                ...
                color: GOLD, fontFamily: "Arial", fontSize: 12, cursor: "pointer",
          ...
          <div style={{ padding: "16px 16px 0" }}>
            <div style={{ fontSize: 10, color: MUTED, fontFamily: "Arial", letterSpacing: 1, marginBottom: 6 }}>
              SIGNED IN AS
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <div style={{ fontSize: 13, color: GOLD, fontFamily: "Arial" }}>✦ {userName}</div>
              <button onClick={() => setUserName("")} style={{ background: "none", border: "none", color: MUTED, fontSize: 10, cursor: "pointer", fontFamily: "Arial" }}>
                Switch
              </button>
            </div>
            {activity.length > 0 && (
              <>
                <div style={{ fontSize: 10, color: MUTED, fontFamily: "Arial", letterSpacing: 1, marginBottom: 8 }}>RECENT ACTIVITY</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {activity.slice(0, 5).map((a, i) => (
                    <div key={i} style={{ fontSize: 11, fontFamily: "Arial", color: MUTED, lineHeight: 1.5 }}>
          ...
          <div style={{ padding: "16px 16px 0", fontSize: 10, color: MUTED, fontFamily: "Arial", letterSpacing: 1 }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "40px 36px 80px" }}>
          ...
          {!activeDash ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", gap: 16 }}>
              ...
              <div style={{ fontSize: 24, color: CREAM, fontStyle: "italic" }}>Your dashboards live here</div>
              <div style={{ fontSize: 14, color: MUTED, fontFamily: "Arial", textAlign: "center", maxWidth: 320, lineHeight: 1.7 }}>
              ...
              <button onClick={() => { setForm({ name: "", emoji: "✨", color: GOLD, sub: "" }); setShowNewDash(true); }}
                style={{
                  ...
                  color: GOLD, fontFamily: "Arial", fontSize: 14, cursor: "pointer",
                }}>
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 36, gap: 20 }}>
                <div>
                  <div style={{ fontSize: 11, letterSpacing: 4, color: activeDash.color, fontFamily: "Arial", opacity: 0.8, marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
                  ...
                  <div>
                    <div style={{ position: "relative", width: 64, height: 64 }}>
                      ...
                      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center",
                        fontSize: 12, fontFamily: "Arial", fontWeight: "bold", color: activeDash.color }}>
                        {overallPct}%
                      </div>
                    </div>
                    <div style={{ fontSize: 9, color: MUTED, fontFamily: "Arial", textAlign: "center", marginTop: 4 }}>
              ...
              {activeDash.sections.length === 0 ? (
                <div style={{ textAlign: "center", padding: "48px 0", color: MUTED, fontFamily: "Arial", fontSize: 14, lineHeight: 1.7 }}>
              ...
              <button onClick={() => { setSectionForm({ name: "", emoji: "📋" }); setShowNewSection(true); }}
                style={{
                  ...
                  color: MUTED, fontFamily: "Arial", fontSize: 13, cursor: "pointer",
                  transition: "all 0.15s",
                }}>
                + Add section
              </button>
          ...
          {/* Footer */}
          <div style={{ textAlign: "center", marginTop: 60, fontSize: 10, color: MUTED, fontFamily: "Arial", letterSpacing: 2 }}>
      ...
      {confirmDelete && (
        <Modal title="Are you sure?" onClose={() => setConfirmDelete(null)}>
          <p style={{ fontFamily: "Arial", fontSize: 14, color: MUTED, lineHeight: 1.6, margin: "0 0 24px" }}>
